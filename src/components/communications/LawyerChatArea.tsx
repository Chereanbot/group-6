...
export default function LawyerChatArea({ selectedContact, userId }: ChatAreaProps) {
  ...
  const loadMessages = async (pageNum: number = 1) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/lawyer/messages/history?contactId=${selectedContact.id}&page=${pageNum}&limit=20`
      );
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      // Create notification using the API route
      await fetch('/api/notifications/create', {
        ...
        body: JSON.stringify({
          ...
          message: `${data.sender.fullName}: ${message.trim()}`,
          type: 'MESSAGE',
          priority: 'NORMAL',
          link: `/client/communications/messages?contact=${userId}`,
        }),
      });
